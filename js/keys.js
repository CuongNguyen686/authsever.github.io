...
function draw(v){
  ...
  v.innerHTML=`<div class="page-head"><div><h1>Keys</h1><p class="muted">Generate and control production licenses.</p></div>
  <div class="page-actions"><button class="btn" id="copyAll">Copy All</button><button class="btn" id="export">Export</button><button class="btn primary" id="generate">Generate Key</button></div></div>
  <div class="section-card"><div class="toolbar"><input id="search" class="field-input" placeholder="Search key, name, device, package, tweak" value="${esc(state.search)}">
  <select id="status"><option value="">All statuses</option>${statuses.map(s=>`<option ${state.status===s?"selected":""}>${s}</option>`).join("")}</select></div>
  <div class="table-wrap"><table class="data-table"><thead><tr><th>Key</th><th>Name</th><th>Status</th><th>Package</th><th>Tweak</th><th>Device</th><th>Created</th><th>Expires</th><th>Actions</th></tr></thead>
  ...
  <td>${x.expiresAt==="lifetime"?"Lifetime":`${fmtDate(x.expiresAt)}<br><small>${daysLeft(x.expiresAt)}</small>`}</td>
  <td><button class="btn" data-copy="${esc(x.key)}">Copy</button>
  ${x.status!=="Banned"?`<button class="btn danger" data-ban="${esc(x.id)}">Ban</button>`:""}
  <button class="btn" data-edit="${esc(x.id)}">Edit</button><button class="btn danger" data-delete="${esc(x.id)}">Delete</button></td></tr>`).join("")
  :`<tr><td colspan="9"><div class="empty">No keys found.</div></td></tr>`}</tbody></table></div>
  <div class="pagination"><span>${filtered.length} key(s) · Page ${state.page}/${pages}</span><div class="actions"><button class="btn" id="prev">‹</button><button class="btn" id="next">›</button></div></div></div>`;
  ...
  v.querySelectorAll("[data-copy]").forEach(b=>b.onclick=()=>navigator.clipboard.writeText(b.dataset.copy).then(()=>toast("Key copied")));
  v.querySelectorAll("[data-delete]").forEach(b=>b.onclick=async()=>{if(await confirmAction("Delete key","This permanently deletes the key.")){await store.remove("keys",b.dataset.delete);toast("Key deleted");renderKeys(v)}});
  v.querySelectorAll("[data-ban]").forEach(b=>b.onclick=async()=>{
    const x=state.rows.find(k=>k.id===b.dataset.ban); if(!x)return;
    if(await confirmAction("Ban key",`Ban key ${x.key}? This changes its status to Banned.`)){
      await store.update("keys",x.id,{status:"Banned",bannedAt:new Date().toISOString()});
      await navigator.clipboard.writeText(x.key).catch(()=>{});
      toast("Key banned and copied");
      renderKeys(v);
    }
  });
...
async function gen(v){
  ...
  modal("Generate Keys",`<form id="keyForm" class="form-grid">
  <div class="field"><label>Package</label><select name="package" id="packageSelect" required>${packageOptions(packages)}</select></div>
  <div class="field"><label>Key Name</label><input name="keyName" id="keyName" placeholder="Package name"></div>
  <div class="field"><label>Prefix</label><input name="prefix" id="prefix" value="AUTH"></div>
  <div class="field"><label>Length</label><input name="length" type="number" min="5" max="64" value="20"></div>
  <div class="field"><label>Quantity</label><select name="quantity">${[1,10,50,100,500,1000].map(n=>`<option>${n}</option>`).join("")}</select></div>
  <div class="field"><label>Expiration</label><select name="expiration"><option value="1">1 Day</option><option value="3">3 Days</option><option value="7">7 Days</option><option value="30" selected>30 Days</option><option value="90">90 Days</option><option value="365">1 Year</option><option value="lifetime">Lifetime</option></select></div>
  <div class="field"><label>Max Devices</label><input name="maxDevices" type="number" min="1" value="1"></div>
  <div class="field"><label>Tweak</label><input name="tweak"></div>
  <div class="field wide"><label>Note</label><textarea name="note"></textarea></div></form>`,
  async root=>{
    ...
    toast(`${qty} key(s) created`); renderKeys(v);
  });
}

function edit(v,x){
  getPackages().then(packages=>modal("Edit Key",`<form id="editForm" class="form-grid">
  <div class="field"><label>Status</label><select name="status">${statuses.map(s=>`<option ${x.status===s?"selected":""}>${s}</option>`).join("")}</select></div>
  <div class="field"><label>Key Name</label><input name="keyName" value="${esc(x.keyName||"")}"></div>
  <div class="field"><label>Max Devices</label><input name="maxDevices" type="number" min="1" value="${x.maxDevices||1}"></div>
  ...
  <div class="field wide"><label>Note</label><textarea name="note">${esc(x.note||"")}</textarea></div></form>`,
  async root=>{
    ...
    toast("Key updated"); renderKeys(v);
  }));
}
